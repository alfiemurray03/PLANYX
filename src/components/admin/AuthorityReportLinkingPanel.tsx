...
export default function AuthorityReportLinkingPanel({
  ...
  async function searchUsers(): Promise<void> {
    ...
    if (value.length < 2) {
      setError('Enter at least two characters from the customer’s name or email address.');
      return;
    }
    setSearching(true);
    setError('');
    setMessage('');
    try {
      const response = await fetch(`/api/admin/authority-report-context?q=${encodeURIComponent(value)}`, { credentials: 'include', cache: 'no-store' });
      const data = await response.json().catch(() => ({})) as { success?: boolean; users?: UserSearchResult[]; error?: string };
      if (!response.ok || !data.success) throw new Error(data.error || 'Customer search failed.');
      setResults(data.users || []);
      if (!(data.users || []).length) setMessage('No customer account matched that name or email address.');
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Customer search failed.');
    } finally {
      setSearching(false);
    }
  }
  ...
  async function selectUser(result: UserSearchResult): Promise<void> {
    ...
    setMessage('');
    try {
      const response = await fetch(`/api/admin/authority-report-context?email=${encodeURIComponent(result.email)}`, { credentials: 'include', cache: 'no-store' });
      const data = await response.json().catch(() => ({})) as { success?: boolean; user?: ReportContextUser; error?: string };
      if (!response.ok || !data.success || !data.user) throw new Error(data.error || 'Customer context could not be loaded.');
      ...
      setError(reason instanceof Error ? reason.message : 'Customer context could not be loaded.');
    } finally {
      setLoadingUser('');
    }
  }
    ...
    <div className="space-y-5">
      <Card className="border-blue-200 bg-white dark:border-blue-500/30 dark:bg-slate-900">
        <CardContent className="space-y-4 p-4 sm:p-5">
          <div>
            <div className="flex items-center gap-2"><CircleUserRound className="h-5 w-5 text-blue-700 dark:text-blue-300" /><h3 className="font-black text-slate-950 dark:text-white">Search and link a customer</h3></div>
            <p className="mt-1 text-xs leading-5 text-slate-600 dark:text-slate-300">Search the live customer directory by name, email or organisation. Selecting a customer does not change the report until you press <strong>Link selected details</strong>.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-slate-400" /><Input value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); void searchUsers(); } }} placeholder="Customer name or email address" className="pl-9" /></div>
          ...
          {results.length > 0 && <div className="max-h-72 space-y-2 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-950/60">{results.map(result => <button key={result.email} type="button" onClick={() => void selectUser(result)} className="flex w-full items-center gap-3 rounded-lg border border-transparent bg-white p-3 text-left transition hover:border-blue-300 hover:bg-blue-50 dark:bg-slate-900 dark:hover:border-blue-500 dark:hover:bg-blue-500/10"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-100 text-sm font-black text-blue-700 dark:bg-blue-500/15 dark:text-blue-200">{initials(result.name)}</div><span className="min-w-0 flex-1"><span className="block font-bold text-slate-950 dark:text-white">{result.name}</span><span className="block truncate text-xs text-slate-500 dark:text-slate-400">{result.email}{result.company ? ` · ${result.company}` : ''}</span><span className="mt-1 block text-[11px] text-slate-500 dark:text-slate-400">{result.accountType} · {result.accountStatus} · {result.addressAvailable ? `Address available${result.postcode ? ` (${result.postcode})` : ''}` : 'No saved address'}</span></span>{loadingUser === result.email ? <Loader2 className="h-4 w-4 animate-spin text-blue-600" /> : <ChevronRight className="h-4 w-4 text-slate-400" />}</button>)}</div>}
        </CardContent>
      </Card>
      ...
      {context.user && <>
        <Card className="border-emerald-200 bg-emerald-50/40 dark:border-emerald-500/30 dark:bg-emerald-500/5">
          <CardContent className="space-y-4 p-4 sm:p-5">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900"><div className="flex items-start gap-3"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-blue-600 dark:text-blue-300" /><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">{context.user.addressSource}</p><p className="mt-1 text-sm font-semibold text-slate-950 dark:text-white">{context.user.address.formatted || 'No postal address is stored for this customer.'}</p>{context.user.stripeCustomerId && <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Stripe customer linked: {context.user.stripeCustomerId}</p>}</div></div></div>
            <div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => void resolvePolice()} disabled={resolvingPolice || !context.user.address.postcode}>{resolvingPolice ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ShieldCheck className="mr-2 h-4 w-4" />}Find responsible force and nearest station</Button><Button type="button" variant="outline" onClick={onOpenFullDirectory}><Building2 className="mr-2 h-4 w-4" />Open full UK station directory</Button></div>
