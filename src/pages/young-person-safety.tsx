...
const safeguards = [
  ...
  { icon: MapPinOff, title: 'Location protection', text: 'Non-essential precise location is off by default. Sousa Murray Planeia does not use a young person’s location to encourage unnecessary sharing.' },
...
export default function YoungPersonSafetyPage() {
  return (
    <>
      <Helmet>
        <title>16+ Safety and Safeguarding — Sousa Murray Planeia</title>
        <meta name="description" content="Sousa Murray Planeia's strict 16+ account rule and safeguards for young people aged 16 and 17." />
      ...
      <div className="bg-background">
        <section className="border-b border-border bg-gradient-to-br from-blue-50 via-white to-violet-50 px-5 py-16 dark:from-slate-950 dark:via-slate-950 dark:to-violet-950/30 sm:px-8 lg:px-10 lg:py-24">
          <div className="mx-auto max-w-5xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-white/80 px-3 py-1.5 text-xs font-black uppercase tracking-[0.12em] text-blue-700 dark:border-blue-500/30 dark:bg-slate-900/80 dark:text-blue-200"><ShieldCheck className="h-4 w-4" /> Sousa Murray Planeia safety</div>
            <h1 className="mt-5 max-w-4xl text-4xl font-black tracking-[-0.045em] text-slate-950 dark:text-white sm:text-5xl lg:text-6xl">Sousa Murray Planeia accounts are strictly for people aged 16 or over.</h1>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600 dark:text-slate-300">Nobody under 16 is permitted to register for or use a Sousa Murray Planeia account. People aged 16–17 can use the service with enhanced privacy and safeguarding protections.</p>
            <div className="mt-7 flex flex-wrap gap-3"><Button asChild><Link to="/sign-in">Go to secure sign-in</Link></Button><Button asChild variant="outline"><Link to="/contact">Contact Sousa Murray Planeia about safety</Link></Button></div>
          </div>
        </section>
        ...
        <div className="mx-auto max-w-6xl space-y-12 px-5 py-12 sm:px-8 lg:px-10 lg:py-16">
          <section>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              ...
              <Card className="border-emerald-200 dark:border-emerald-500/30"><CardContent className="p-5"><p className="text-sm font-black text-emerald-700 dark:text-emerald-200">Age 18+</p><p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Adult account controls apply, subject to Sousa Murray Planeia privacy, security and acceptable-use rules.</p></CardContent></Card>
            </div>
          </section>
