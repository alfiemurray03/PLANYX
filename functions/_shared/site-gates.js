...
export const DEFAULT_GATE_CONFIG = {
  ...
  launch: {
    logoUrl: "",
    statusLabel: "Sousa Murray Planeia is nearly ready",
    ...
    seoTitle: "Something memorable is taking shape — Sousa Murray Planeia",
    seoDescription: "Sousa Murray Planeia is preparing a new way to build experiences, shape plans and create memories.",
  },
  maintenance: {
    logoUrl: "",
    statusLabel: "Maintenance in progress",
    reason: "Planned platform maintenance",
    title: "We are making Sousa Murray Planeia even better",
    message: "Sousa Murray Planeia is temporarily unavailable while our team completes essential improvements.",
    start: "",
    expectedReturn: "",
    timelineEnabled: true,
    contactEnabled: true,
    contactText: "Need help while we are away? Contact contact@jagroupservices.co.uk or call 020 3834 2790.",
    customHtml: "",
    customCss: "",
    ownerEnabled: true,
    ownerPrompt: "Owner of this website?",
    ownerButtonLabel: "SIGN IN HERE",
    ownerUrl: "/admin",
    footerText: `© ${new Date().getFullYear()} Sousa Murray Planeia · Operated by JA Group Services Ltd`,
    legalLinks: LEGAL_LINKS.slice(0, 3),
    seoTitle: "Sousa Murray Planeia maintenance",
    seoDescription: "Sousa Murray Planeia is temporarily unavailable while essential maintenance is completed.",
  },
};
...
export function renderLaunchGate(input) {
  ...
  const features = c.featuresEnabled && c.features.length
    ...
    : "";
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(c.seoTitle)}</title><meta name="description" content="${escapeHtml(c.seoDescription)}"><meta name="robots" content="noindex,nofollow"><meta name="theme-color" content="#060b1a"><style>
...
</style></head><body><div class="page"><header><img src="${escapeHtml(c.logoUrl)}" alt="Sousa Murray Planeia"></header><main><section class="hero"><div class="signal" aria-hidden="true">⌁</div><p class="eyebrow">${escapeHtml(c.statusLabel)}</p><h1>${escapeHtml(c.headline)}${c.highlight ? ` <span>${escapeHtml(c.highlight)}</span>` : ""}</h1><p class="subtext">${escapeHtml(c.subtext)}</p><p class="description">${escapeHtml(c.description)}</p>${countdownMarkup(c)}${features}${c.customHtml ? `<div class="custom">${c.customHtml}</div>` : ""}${ownerMarkup(c)}</section></main><footer><div class="footer-row"><span>${escapeHtml(c.footerText)}</span><nav class="footer-links">${legalMarkup(c.legalLinks)}</nav></div></footer></div>${countdownScript()}</body></html>`;
}
...
export function renderMaintenanceGate(input) {
  ...
  const timeline = c.timelineEnabled && (c.start || c.expectedReturn)
    ...
    : "";
  return `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(c.seoTitle)}</title><meta name="description" content="${escapeHtml(c.seoDescription)}"><meta name="robots" content="noindex,nofollow"><meta name="theme-color" content="#07101f"><style>
...
</style></head><body><div class="shell"><header class="top"><img class="logo" src="${escapeHtml(c.logoUrl)}" alt="Sousa Murray Planeia"><div class="status"><span class="dot"></span>${escapeHtml(c.statusLabel)}</div></header><main><section class="card"><p class="eyebrow">${escapeHtml(c.reason)}</p><h1>${escapeHtml(c.title)}</h1><p class="message">${escapeHtml(c.message)}</p>${timeline}${c.contactEnabled ? `<p class="contact">${escapeHtml(c.contactText)}</p>` : ""}${c.customHtml ? `<div class="custom">${c.customHtml}</div>` : ""}${ownerMarkup(c)}</section></main><footer><span>${escapeHtml(c.footerText)}</span><nav>${legalMarkup(c.legalLinks)}</nav></footer></div>${countdownScript()}</body></html>`;
}
